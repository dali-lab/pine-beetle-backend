...
// download endpoint for data export
summarizedRDRouter.route('/download')
  .get(async (req, res) => {
    const validQueryFields = [
      'rangerDistrict',
      'state',
    ...
    try {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="summarized-ranger-district-${startYear || 'all'}-${endYear || 'all'}.csv"`);
